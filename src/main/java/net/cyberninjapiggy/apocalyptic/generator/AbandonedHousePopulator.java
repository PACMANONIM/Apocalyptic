    ...
    for (int i = 0; i <= houseX; i++) {
      for (int o = 0; o <= houseZ; o++) {
        for (int p = 0; p < 5; p++) {
          ...
          boolean isCorner =
              (i == 0 && o == 0) || (i == 0 && o == houseZ) || (i == houseX && o == 0)
                  || (i == houseX && o == houseZ);
          boolean isDoorSpot =
              (door == 0 && i == midX && o == 0) || (door == 1 && i == midX && o == houseZ)
                  || (door == 2 && i == 0 && o == midZ) || (door == 3 && i == houseX && o == 0);
          if (p == 0) {
            if (isWall && !isDoorSpot) {
              ...
              world.getBlockAt(realX + i, realY + p, realZ + o).setTypeIdAndData(
                  Material.WOOD.getId(), (byte) 3, false);
            ...
            if (i == 0) {
              // noinspection deprecation,deprecation
              world.getBlockAt(realX + i, realY + p, realZ + o).setTypeIdAndData(
                  Material.COBBLESTONE_STAIRS.getId(), (byte) 0, false);
            } else if (i == houseX) {
              // noinspection deprecation,deprecation
              world.getBlockAt(realX + i, realY + p, realZ + o).setTypeIdAndData(
                  Material.COBBLESTONE_STAIRS.getId(), (byte) 1, false);
            } else if (o == 0) {
              // noinspection deprecation,deprecation
              world.getBlockAt(realX + i, realY + p, realZ + o).setTypeIdAndData(
                  Material.COBBLESTONE_STAIRS.getId(), (byte) 2, false);
            } else if (o == houseZ) {
              // noinspection deprecation,deprecation
              world.getBlockAt(realX + i, realY + p, realZ + o).setTypeIdAndData(
                  Material.COBBLESTONE_STAIRS.getId(), (byte) 3, false);
            } else {
              if (rand.nextBoolean())
                world.getBlockAt(realX + i, realY + p, realZ + o).setType(Material.WOOD);
              else
            ...
            if (!chest && rand.nextInt((houseX - 1) / 2 * (houseZ - 1) / 2) == 0) {
              ...
              chestPopulator.populateChest(new Location(world, realX + i, realY + p, realZ + o),
                  rand);
              chest = true;
            } else {
              world.getBlockAt(realX + i, realY + p, realZ + o).setType(Material.AIR);
