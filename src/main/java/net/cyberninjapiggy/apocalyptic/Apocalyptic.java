...
public final class Apocalyptic extends JavaPlugin {
	...
	public void onEnable() {
		...
		try {
			if (db.isTable("radiationLevels")) {
				...
				if (toUpdate.size() > 0) {
					...
					db.query("CREATE TABLE radiationLevels (" + "id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT," + "player VARCHAR(36)," + "level DOUBLE);");
        ...
        */
		...
		for (int i = 0; i <= 3; i++) {
			...
			for (int j = 0; j <= 3; j++) {
				int mId = start + (i * 4) + j;@SuppressWarnings("deprecation") Material mat = Material.getMaterial(mId);
				...
				recipe.addIngredient(mat);@SuppressWarnings("deprecation") Material chain = Material.getMaterial(start - 4 + j);
		...
		start = 298;
		for (int j = 0; j <= 3; j++) {
			int mId = start + j;@SuppressWarnings("deprecation") Material mat = Material.getMaterial(mId);
			ShapelessRecipe recipe = new ShapelessRecipe(Util.setName(new ItemStack(mat), ChatColor.RESET + messages.getCaption("hazmat") + " " + Util.title(mat.name().replace("_", " ").toLowerCase())));
			recipe.addIngredient(mat);@SuppressWarnings("deprecation") Material chain = Material.getMaterial(mId + 4);
			recipe.addIngredient(chain);
		...
		getServer().getScheduler().scheduleSyncRepeatingTask(this, new Runnable() {@Override
			public void run() {
				for (World w: getServer().getWorlds()) {
					...
					for (Player p: w.getPlayers()) {
						...
						if (!noFallout && (worldEnabledFallout(w.getName()) || forceFallout)) {
							...
							if (p.getEquipment().getHelmet() == null && p.getWorld().getHighestBlockYAt(l.getBlockX(), l.getBlockZ()) <= l.getBlockY() && p.getWorld().hasStorm() && temp < 1.0D) {
	...
	public boolean canDoCommand(CommandSender p, String cmd) {
		...
		if (usePerms) {
			return (cmd.equals("radiation.self") && p.hasPermission("apocalyptic.radiation.self")) || (cmd.equals("radiation.other") && p.hasPermission("apocalyptic.radiation.other")) || (cmd.equals("radiation.change") && p.hasPermission("apocalyptic.radiation.change.self")) || (cmd.equals("apocalyptic.radhelp") && p.hasPermission("apocalyptic.help.radiation")) || (cmd.equals("apocalyptic.stop") && p.hasPermission("apocalyptic.admin.stop")) || (cmd.equals("apocalyptic.reload") && p.hasPermission("apocalyptic.admin.reload") || (cmd.equals("hazmatArmor.self") && p.hasPermission("apocalyptic.hazmatArmor.self")) || (cmd.equals("hazmatArmor.other") && p.hasPermission("apocalyptic.hazmatArmor.other")));
		} else {
