...
public class ZombieTarget implements Listener {
	...
	public void onEntityTarget(EntityTargetEvent e) {
		if (!(e.getEntityType() == EntityType.ZOMBIE) || !(plugin.worldEnabledZombie(e.getEntity().getWorld().getName()))) {
			return;
		}
		if (e.getTarget() == null) {
			double searchRadius = plugin.getConfig().getWorld(e.getEntity().getWorld()).getDouble("mobs.zombies.targetRange") * 2;
