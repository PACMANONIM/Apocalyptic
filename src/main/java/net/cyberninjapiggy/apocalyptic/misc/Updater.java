...
public class Updater {
  ...
  private static final String HOST = "https://api.curseforge.com"; // Slugs will be appended to this
                                                                   // to get to the project's RSS
                                                                   // feed
  ...
  private void saveFile(File folder, String file, String u) {
    ...
    try {
      ...
      for (final File xFile : new File(this.plugin.getDataFolder().getParent(), this.updateFolder)
          .listFiles()) {
  ...
  private void unzip(String file) {
    try {
      ...
      // Move any plugin data folders that were included to the right place, Bukkit won't do this
      // for us.
  ...
  private boolean versionCheck(String title) {
    if (this.type != UpdateType.NO_VERSION_CHECK) {
      ...
      if (title.split(" v").length == 2) {
        final String remoteVersion = title.split(" v")[1].split(" ")[0]; // Get the newest file's
                                                                         // version number
        ...
        final String authorInfo =
            this.plugin.getDescription().getAuthors().size() == 0 ? "" : " ("
